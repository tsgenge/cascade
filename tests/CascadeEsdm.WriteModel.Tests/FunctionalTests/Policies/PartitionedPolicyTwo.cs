...
public class PartitionedPolicyTwo : IPolicy
{
    private readonly ICommandHandler<PartitionedPolicyTwoExecuted> _handler;

    public PartitionedPolicyTwo(ICommandHandler<PartitionedPolicyTwoExecuted> handler)
    {
    ...
    {
        await _handler.HandleAsync(new CommandEnvelope<PartitionedPolicyTwoExecuted>(
            new PartitionedPolicyTwoExecuted(),
