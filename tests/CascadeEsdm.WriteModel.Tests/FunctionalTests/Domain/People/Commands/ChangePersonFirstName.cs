...
{
    public async IAsyncEnumerable<IEventEnvelope> ExecuteAsync(ICommandEnvelope<ChangePersonFirstName> envelope, PersonAggregate aggregate)
    {
        if(!aggregate.Exists)
    ...
    public Task<ISecurityDescriptor?> GetSecurityDescriptorAsync(ICommandEnvelope<ChangePersonFirstName> envelope, PersonAggregate aggregate)
    {
