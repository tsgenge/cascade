...
{
    public async IAsyncEnumerable<IEventEnvelope> ExecuteAsync(ICommandEnvelope<ChangePersonLastName> envelope, PersonAggregate aggregate)
    {
        if(!aggregate.Exists)
    ...
    public Task<ISecurityDescriptor?> GetSecurityDescriptorAsync(ICommandEnvelope<ChangePersonLastName> envelope, PersonAggregate aggregate)
    {
